...
<div class="container">
    ...
    @include('errors.success')
        <div style="float: left; margin-bottom: 10px; margin-top: 20px;">
            <a href="{{ route('departments.index') }}" class="btn btn-success"><i class="fa fa-chevron-circle-left"></i> {{ trans('label.back') }}</a>
        </div>
        @if(Auth::user()->hasRole('admin') || (Auth::user()->hasRole('manager') && Auth::user()->id_department === $department->id))
        <div style="float: right; margin-bottom: 10px; margin-top: 20px;">
            <a class="btn btn-primary" href="{{ route('admin.departments.users.create', $department->id) }}" role="button"><i class="fa fa-plus-circle"></i> {{ trans('label.departments.add_user') }}</a>
        </div>
        @endif
